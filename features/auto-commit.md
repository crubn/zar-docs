...
By default ZAR proposes doc changes as a [pull request](pr-feedback.md). **Auto-commit** lets it skip the PR and write the fix directly. It's powerful and intentionally hard to enable by accident.
...
Auto-commit suits teams that have watched ZAR's PRs for a while and trust the output, especially with a [style guide](../configuration/style-guide.md) dialed in. A good rollout:
...
- **Dry-run wins.** A repo in [dry-run](../configuration/trigger-modes.md#dry-run) never commits, even with auto-commit on.
- **`disable_pr`** stops ZAR from opening PRs or committing entirely — a hard off switch.
- Auto-commit doesn't change [significance or trigger](../configuration/trigger-modes.md) decisions; it only changes *how* an approved change is delivered.

## Next steps

- **[Commands](commands.md)** — `enable auto-commit`, `status`.
- **[Safety & guarantees](../concepts/safety-and-guarantees.md)** — why writes need two locks.
- **[Environment variables](../reference/environment-variables.md)** — `DOCAGENT_WRITE_COMMITS` (self-hosting).
