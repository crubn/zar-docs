...
Most problems fall into a handful of buckets. Find your symptom below. When in doubt, open the [run view](features/dashboard.md#run-view) for the PR or push — its timeline usually shows exactly which step stopped.
...
- **Did the PR change real behavior?** Lock-file-only, test-only, and config-only diffs are skipped by design. Try a change that adds a public function, endpoint, or config flag. See [significance](configuration/trigger-modes.md#the-significance-filter).
- **Permissions present?** Pull requests (R/W), Issues (R/W), Contents (R/W), Checks (R/W), Metadata (R). See [Permissions & events](reference/permissions-and-events.md).
...
- **Subscription required?** If the run summary says "Subscription required", see [Plans](plans.md).
...
- For hard enforcement, add the `docagent/ci-gate` check to **branch protection**. See [CI gate](features/ci-gate.md).
...
If either is missing, ZAR opens a PR instead. Also check the repo isn't in **dry-run** (which forbids commits). See [Auto-commit](features/auto-commit.md).

## ZAR's edit looks wrong or off-voice

- Set a [style guide](configuration/style-guide.md) so edits match your conventions, then push again.
...
You're likely on SQLite under concurrency. Switch to **Postgres** (`DATABASE_URL=postgresql+asyncpg://…`) for production. SQLite is fine for local testing only. See [Self-hosting](self-hosting/overview.md#database).

## Duplicate docs PRs

ZAR keeps at most one open docs PR per base branch on a single instance. If you see duplicates, you're probably running **multiple replicas** without a shared lock — see the [scaling note](self-hosting/overview.md#scaling-note).

## Still stuck?

- Open the [run view](features/dashboard.md#run-view) and read the timeline and `error` field.
